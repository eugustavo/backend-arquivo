...
export async function cfopResumo(
    ...
    for (let i = 0; i < empresasAnalista.length; i++) {
        ...
        let dados: any = await CfopProdutosDetalhe(converteData(data1), converteData(data2), empresasAnalista[i].CODIGOEMPRESA, empresasAnalista[i].CODIGOESTAB)
    ...
    if (todos) {
        ...
        for (let i = 0; i < empresasAtivas.length; i++) {
            ...
            let dados: any = await CfopProdutosDetalhe(converteData(data1), converteData(data2), empresasAtivas[i].CODIGOEMPRESA, empresasAtivas[i].CODIGOESTAB)
